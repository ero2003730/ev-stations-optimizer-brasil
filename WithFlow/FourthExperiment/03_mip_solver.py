...
def main():
    ...
    # 1) carrega grafo, demanda e fluxo (ignora supply_existing)
    ...
    # 3) associa VMD do ponto de fluxo mais próximo
    ...
    # garante que ambos estejam em EPSG:4326
    if ufs.crs is None:
    ...
    out.to_parquet(OUTPUT_DIR/"sites_new_mip_withflow12.parquet", index=False)

    dt = time.perf_counter() - t0_all
    print(f"✓ Gerado {len(chosen)} estações em {dt:.1f}s")
